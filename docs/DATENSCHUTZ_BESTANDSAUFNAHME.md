...
`fe/app/beitragskarte/beitragskarte.component.html:186` (seit der Beitragskarte eine Stelle
für alle Typen; vorher je Typ in `*-result-item.component.html`).

Angezeigt wurde bis zur Beitragskarte der Rohwert des `X-User`-Headers, also die Keycloak-`sub`
(eine UUID) bzw. `user-001`/`test-user-id-1`; seitdem sind es die ersten 8 Zeichen davon
(`fe/app/shared/kennung.ts`, bis es Anzeigenamen gibt) — **kein Klarname**. Über die API geht
weiterhin die volle Kennung. Für Betroffene bleibt es eine dauerhaft öffentliche, über alle
Beiträge hinweg verknüpfbare Kennung.
...
|------|-------------|--------|-------|-------------|
| `ContentGruenAuthCookie` (Keycloak-Modus) | `BFF/Program.cs:74` | ASP.NET-Cookie-Auth-Ticket, DataProtection-verschlüsselt; enthält **alle Keycloak-Claims** (`sub`, `email`, Name — Scopes `openid`,`profile`,`email`, `Program.cs:103-105`), **keine Tokens**, da `options.SaveTokens = false` (`:102`) | `HttpOnly` (`:79`), `Secure = Always` (`:78`), `SameSite = None` (`:77`) | **14 Tage, gleitend**, persistent — `ExpireTimeSpan = TimeSpan.FromDays(14)`, `SlidingExpiration = true` (`Program.cs:92-93`), `IsPersistent = true` in `OnTicketReceived` (`:124`) |
| `ContentGruenAuthCookie` (Managed-Modus) | `BFF/Program.cs:142` | Claims `NameIdentifier`, `Name`, `name`, `Email`, `sub`, `auth_method`, `user_id`, ggf. `Role=admin`/`isAdmin` (`AuthController.cs:91-104`) | `HttpOnly` (`:147`), `SecurePolicy = SameAsRequest` (`:146` — **über http also ohne `Secure`**), `SameSite = Lax` (`:145`) | **14 Tage, gleitend**, persistent — Laufzeit vom Cookie-Handler (`Program.cs:149-150`), `IsPersistent = true` ohne eigenes `ExpiresUtc` (`AuthController.cs:110-112`) |
...
> **„8 Stunden ab der letzten Nutzung"** — gleitend, nicht absolut. Seit PR #49 **14 Tage**;
> Formulierung **„bis zu 14 Tage nach der letzten Nutzung"** — der Handler verlängert erst nach
> Ablauf der halben Laufzeit, real gilt das Cookie also 7–14 Tage ab der letzten Nutzung.
...
| 2-B | **beliebige Bild-Hosts** | IP + User-Agent des **Betrachters** gehen an den Host des jeweiligen Bildes | `fe/app/beitragskarte/beitragskarte.component.html` hat **zwei** `<img [src]>`-Stellen mit derselben Adresse: die volle Karte `:133` (`daten.bildUrl`) und das Kopfband der Album-Karte in Meine Beiträge `:9` (`kopfBild`, lazy geladen); vorher `image-result-item.component.html:41`. Wert stammt aus `domain/models/image.py:37`, vom Beitragenden frei eingegeben (`fe/app/add-image/add-image.component.html:34`) | abhängig vom eingetragenen Host — nicht vorhersagbar | **ja**, sobald ein Bildbeitrag im Suchergebnis erscheint. Suche ist öffentlich → betrifft auch nicht angemeldete Besucher. Zusätzlich in Meine Beiträge (nur angemeldet, eigene Bilder) |
...
| Wert | Konsequenz |
|------|-----------|
| `true` | Anmeldung über Netzbegrünung-Keycloak. Claims `sub`, `email`, Name kommen ins Auth-Cookie, die Tokens nicht (`SaveTokens = false`, `Program.cs:102`). Der Reverse-Proxy erhält eine Autorisierungspipeline (`Program.cs:491-509`). **Für die Erklärung:** IdP benennen, Verantwortlichkeitsabgrenzung/AVV klären |
