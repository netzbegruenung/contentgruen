...
| D-N13b | **Bild-Adressen Dritter** | `fe/app/beitragskarte/beitragskarte.component.html` mit zwei `<img [src]>`-Stellen: volle Karte `:133`, Kopfband der Album-Karte in Meine Beiträge `:9` (vorher `image-result-item.component.html:41`), `domain/models/image.py:37` | Der einzige Abruf, den der Browser der Nutzenden an einen fremden Server richtet — siehe X4 | ✓ Eigener Abschnitt „Bilder von fremden Servern" |
